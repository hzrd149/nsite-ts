...
(() => {
  ...
  function getShortName(pubkey, metadata) {
    let name = metadata.name || metadata.display_name;
    if (name)
      return name;
  ...
  function shortenNpub(npub) {
    return npub.substring(0, 7) + "\u2026" + npub.substring(57);
  }
    ...
    set = (0, import_debounce.default)(async () => {
      ...
      if (input) {
        ...
        if (pubkey) {
          ...
          this.textContent = getShortName(pubkey, metadata);
        }
      }
    }, 200);
  };
